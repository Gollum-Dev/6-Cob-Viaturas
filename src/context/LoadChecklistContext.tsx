...
export function LoadChecklistProvider({ children }: { children: ReactNode }) {
  ...
  const fetchLoadSubmissions = async () => {
    ...
    if (!error && data) {
      ...
      // If not ADMINISTRADOR, filter by allowed maps of this unit
  ...
  return (
    <LoadChecklistContext.Provider value={{ loadSubmissions, isLoading, addLoadSubmission, refreshLoadSubmissions: fetchLoadSubmissions }}>
      {children}
