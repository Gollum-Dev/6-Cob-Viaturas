...
export default function EditMilitarForm() {
  const { registeredUsers, updateUser, user: currentUser } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check if milNumber is changed and if new milNumber already exists
    const existingUser = registeredUsers.find(u => u.id === id);
    if (existingUser && existingUser.milNumber !== milNumber) {
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-on-surface uppercase tracking-tight flex items-center gap-2 sm:gap-3">
             <Award className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 text-primary flex-shrink-0" />
             Editar Militar
          </h1>
          <p className="text-[10px] sm:text-xs md:text-sm text-on-surface-variant font-medium mt-1">Atualizar informações e permissões do militar.</p>
        </div>
      </div>
